...
export function MySigmaClient({ stocks, snapshot, sharedSymbols = "" }: MySigmaClientProps) {
  ...
  const [sigmaFilter,setSigmaFilter] = React.useState<SigmaFilter>("all");
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-[1600px] flex-1 px-4 pt-10 pb-4 sm:px-6 sm:pt-14 lg:px-8">
        ...
        <div id="symbol-search" className="mt-10 scroll-mt-20 space-y-10">
          <Section
          ...
          >
            {ready && watchlist.length > 0 && !watchlist.some(stock => matchesSigmaFilter(stock,sigmaFilter)) && <p className="py-4 text-sm text-muted-foreground">{pick("선택한 조건에 해당하는 종목이 없습니다.", "No symbols match this filter.")}</p>}
            {shared && (
            ...
            {!ready ? (
              ...
              <div className={GRID}>
                {watchlist.filter(stock => matchesSigmaFilter(stock,sigmaFilter)).map((stock) => (
