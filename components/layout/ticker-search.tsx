...
export function TickerSearch({ items, expandedWidth = false }: { items: readonly TickerSearchItem[]; expandedWidth?: boolean }) {
  ...
  return (
    <div className={cn("relative min-w-0", expandedWidth ? "w-full" : "flex-1 sm:w-44 sm:flex-none xl:w-48")}
      ...
      <input ref={input} role="combobox" aria-autocomplete="list" aria-expanded={expanded}
        ...
        aria-label={pick("종목명 또는 티커 검색", "Search symbol or company")} placeholder={pick("종목명 또는 티커 검색", "Symbol or company")}
        autoComplete="off" autoCapitalize="characters" spellCheck={false} value={query}
        className="h-11 md:h-8 w-full min-w-0 rounded-lg border border-border/70 bg-background/50 pr-12 pl-7 text-base outline-none placeholder:text-muted-foreground focus:border-ring focus:ring-2 focus:ring-ring/25 sm:text-sm"
        ...
        onKeyDown={(event) => {
          ...
          if (event.key === "Enter" && expanded && matches[selected]) {
            event.preventDefault(); rememberBoardScroll(); router.push(href(matches[selected].symbol)); close();
          }
        }} />
      {query && <button type="button" aria-label={pick("검색 초기화", "Clear search")}
        onClick={() => { close(); input.current?.focus(); }}
        className="absolute top-1/2 right-0 flex size-11 md:size-8 items-center justify-center -translate-y-1/2 rounded text-muted-foreground hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring">
        <X aria-hidden className="size-3.5" />
      </button>}
      {expanded && <div className="absolute inset-x-0 top-full z-50 mt-2 overflow-hidden rounded-xl border border-border bg-popover text-popover-foreground shadow-xl md:inset-x-auto md:right-0 md:w-80">
        ...
        <ul ref={list} id={`${id}-list`} role="listbox" aria-label={pick("티커 검색 결과", "Ticker search results")} className="max-h-[min(18rem,45dvh)] overflow-y-auto overscroll-contain py-1">
