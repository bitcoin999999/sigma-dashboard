...
export function BandDiagram({
  ...
  const copy = GUIDE_COPY[lang].band;

  const ticks = [
    { at: EDGE.lowerExtreme, label: "−1.5σ" },
    { at: EDGE.lower1, label: "−1σ" },
    { at: EDGE.anchor, label: copy.anchorTick },
  ...
  return (
    <figure className={cn("w-full", className)}>
      ...
      <div className="relative h-3" role="img" aria-label={copy.ariaLabel}>
      ...
      <figcaption className="mt-4 flex justify-between gap-4 text-[11px] leading-relaxed text-muted-foreground">
        <span className="max-w-[13rem] text-left">{copy.below}</span>
        <span className="hidden max-w-[13rem] text-center sm:block">
          {copy.within}
        </span>
        <span className="max-w-[13rem] text-right">{copy.above}</span>
      </figcaption>
    </figure>
  );
}
