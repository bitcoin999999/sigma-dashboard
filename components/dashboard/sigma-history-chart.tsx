...
import { DEFAULT_PRICE_SIGMA_PERIOD, PRICE_SIGMA_PERIODS, priceSigmaPeriodStart, type PriceSigmaPeriod } from "@/lib/price-sigma-chart";
...
export function SigmaHistoryChart({ history, session }: { history: MarketHistory; session: string }) {
  ...
  const [period,setPeriod] = useState<PriceSigmaPeriod>(DEFAULT_PRICE_SIGMA_PERIOD);
  const [side,setSide] = useState<"all"|"upper"|"lower">("all");
  const start = priceSigmaPeriodStart(session, period);
  const rows = historyChartRows(history).filter(r => r.date >= start && r.date <= session);
  const bandCount = rows.filter(row => row.sigmaPosition !== null).length;
  ...
  return <section className="glass mt-8 min-w-0 rounded-2xl p-4 sm:p-6" aria-label={pick("주가와 주간 시그마 이력", "Price and weekly sigma history")}>
    ...
    <div className="mt-2 space-y-1 text-[11px] leading-relaxed text-muted-foreground">
      ...
      <p>{bandCount ? pick(`σ 밴드 ${bandCount}/${rows.length}거래일${bandCount < rows.length ? " · 없는 날짜는 주가만 표시" : ""}`, `Sigma bands: ${bandCount}/${rows.length} sessions${bandCount < rows.length ? " · missing bands show price only" : ""}`) : pick("이 구간은 과거 σ 밴드가 없어 주가만 표시합니다.", "No historic sigma bands in this period; only price is available.")}</p>
    </div>
